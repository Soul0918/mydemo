...
use Common\Model\UserLoginHistModel;
use Org\Util\Date;
use Think\Log;

class PublicController extends OperatebaseController {

    /**
     * @var UserLoginHistModel
     */
    protected $user_login_hist_model;

    public function _initialize()
    {
        // parent::_initialize();
        C(S('sp_dynamic_config'));//加载动态配置
        $this->user_login_hist_model = D('UserLoginHist');
    }
    
    //后台登陆界面
    public function login() {
        $admin_id=session('ADMIN_ID');
    	if(!empty($admin_id) && empty(session('LOGIN_TYPE'))){//已经登录
//            redirect(U('admin/index/index'));
            header('Location:index.php?g=operate&m=index&a=welcome');
    	}else{
    ...
    public function dologin(){
    	...
    	$name = I("post.login-name");
    	if(empty($name)){
    		$this->error(L('USERNAME_OR_EMAIL_EMPTY'));
    	}
    	$pass = I("post.login-pass");
    	if(empty($pass)){
    	...
    	if(!sp_check_verify_code()){
    		...
    		if(!empty($result) && $result['user_type']==1){
    			if(sp_compare_password($pass,$result['user_pass'])){
                    ...
                    $this->user_login_hist_model->history(sp_get_current_admin_id(),sp_get_current_admin_id(),5);
    				$this->success(L('LOGIN_SUCCESS'),U("operate/index/welcome"));
    			}else{
