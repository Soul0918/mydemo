...
class ManagmentbaseController extends AppframeController
{
    const LOGIN_COMPANY = 1;

    const LOGIN_COMMUNITY = 2;

    protected $app_menu = array('Managment', 'Admin');
    ...
    {
        hook('admin_begin');
        $managmenttpl_path = C("SP_MANAGMENT_TMPL_PATH") . C("SP_MANAGMENT_DEFAULT_THEME") . "/";
        C("TMPL_ACTION_SUCCESS", $managmenttpl_path . 'Managment/success.html');
        C("TMPL_ACTION_ERROR", $managmenttpl_path . 'Managment/error.html');
        $time = time();
    ...
    function _initialize()
    {
        ...
        //暂时取消后台多语言
        $this->no_need_check_rules = array_merge($this->no_need_check_rules, array("ManagmentIndexindex", "ManagmentMainindex", "AdminUseruserinfo", "AdminSettingpassword","ManagmentPublicNoticebroadapi"));
//        $this->load_app_admin_menu_lang();
        $this->load_menu_lang();
        $this->_check_access();
        $this->_check_is_admin();
        $this->assign('menus', session('admin_menu_' . sp_get_current_admin_id()));
     ...
     * 获取物业id和小区id
     */
    public function get_companyid_communityid( $community_id = 0, $company_id=0 ){
    	if ($this->login_type) {
    		if ($this->login_type == 1) {
    			$company_id     = sp_get_current_company_id();
    		} elseif ($this->login_type == 2) {
    			$community_id   = sp_get_current_community_id();
    			$company_id   = D('Communities')->field('company_id')->find($community_id);
    		}
    	}
...
//        $no_need_check_rules=array("ManagmentIndexindex","ManagmentMainindex","AdminUseruserinfo", "AdminSettingpassword");
