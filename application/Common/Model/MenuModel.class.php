...
class MenuModel extends CommonModel {
    ...
    public function get_menu($parentid, $with_self = false)
    {
        $login_type = session('LOGIN_TYPE');
        //父节点ID
        $parentid = (int) $parentid;
        $query = $this->alias('a')->field('a.*')->where(array('a.parentid' => $parentid, 'a.status' => 1))->order(array("a.listorder" => "ASC"));
        if (!empty($login_type)) {
            $var = '';
            $login_type == 1 && $var = ' and b.is_company = 1';
            $login_type == 2 && $var = ' and b.is_community = 1';
            $query->join('__MODULES__ b on a.module_id = b.module_id'.$var);
        }
...
//        $data = !session('COMPANY_ID') || (session('COMPANY_ID') && session('IS_NOT_COMAPNY_ADMIN')) ? $this->admin_menu($myid) : $this->company_menu($myid);;
//        $data = $this->admin_menu($myid);
        $data = $this->get_menu($myid);
        $Level++;
        if (is_array($data)) {
            $ret = NULL;
            foreach ($data as $a) {
     ...
     * @return array
     */
    private function _check_auth($result, $type='')
    {
        $app_menu = C('APP_MENU');
//        if (!empty($app_menu)) {
//            $query->where(['app' => ['in', implode(',', $app_menu)]]);
//        }
        $array = array();
        foreach ($result as $v) {
            //方法
            $action = $v['action'];
            if (!in_array($v['app'], $app_menu, true)) continue;
            //public开头的通过
            if (preg_match('/^public_/', $action)) {
                $array[] = $v;
            } else {
