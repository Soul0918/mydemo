...
class TagLibSpadmin extends TagLib {
    ...
    protected $tags = array(
        ...
        'managmenttpl' => array("attr" => "file", "close" => 0),
    );
     ...
     * @return array
     */
    public function _managmenttpl($tag, $content) {
        $file = $tag['file'];
        $counts = count($file);
        if ($counts < 3) {
            $file_path = "Managment" .  "/" . $tag['file'];
        } else {
            $file_path = $file[0] . "/" . "Tpl" . "/" . $file[1] . "/" . $file[2];
        }
        //模板路径
        $TemplatePath = sp_add_template_file_suffix( C("SP_MANAGMENT_TMPL_PATH") .C("SP_MANAGMENT_DEFAULT_THEME")."/". $file_path );
        //判断模板是否存在
        if (!file_exists_case($TemplatePath)) {
            return false;
        }
